...
'''
1. REGULARIZER = 0.00001, LEARNING_RATE = 0.0001 
2. REGULARIZER = 0.00001, LEARNING_RATE = 0.00001 - loss: 0.9101 - accuracy: 0.7050
3. REGULARIZER = 0.00001, LEARNING_RATE = 0.001 - loss: 0.1693 - accuracy: 0.9850
4. REGULARIZER = 0.0001, LEARNING_RATE = 0.0001 - loss: 1.2853 - accuracy: 0.9580
5. REGULARIZER = 0.001, LEARNING_RATE = 0.0001 - loss: 1.2853 - accuracy: 0.9580
6. 
'''
...
def create_alexnet(in_shape=(IMAGE_SIZE, IMAGE_SIZE, COLOR), n_classes=CLASSES,
                   kernel_regular=regularizers.l2(l2=REGULARIZER), optimizer='adam'):
    input_tensor = Input(shape=in_shape)

    # Layer 1
    x = Lambda(lambda image: tf.image.per_image_standardization(image))(input_tensor)
    x = Conv2D(filters=96, kernel_size=(11, 11), strides=(4, 4), padding="valid", kernel_regularizer=kernel_regular)(x)
    x = Activation('relu')(x)
    ...
    # Layer 6
    x = Flatten()(x)
    x = Dense(units=4096, activation='relu', kernel_regularizer=kernel_regular)(x)
    x = Dropout(0.5)(x)
...
# 곱셈의 형태로 ndarray 만들기
train_images = np.zeros(number_of_train_data * IMAGE_SIZE * IMAGE_SIZE * COLOR, dtype=np.int32).reshape(
    number_of_train_data, IMAGE_SIZE, IMAGE_SIZE, COLOR)
train_labels = np.zeros(number_of_train_data, dtype=np.int32)
test_images = np.zeros(number_of_test_data * IMAGE_SIZE * IMAGE_SIZE * COLOR, dtype=np.int32).reshape(
    number_of_test_data, IMAGE_SIZE, IMAGE_SIZE, COLOR)
...
(tr_images, tr_oh_labels), (val_images, val_oh_labels), (test_images, test_oh_labels) = \
    get_train_valid_test_set(train_images, train_labels, test_images, test_labels, valid_size=VALID_SIZE,
                             random_state=2021)

print(train_labels.shape)
print(tr_images.shape, tr_oh_labels.shape, val_images.shape, val_oh_labels.shape, test_images.shape,
...
tr_ds = Alphabet_Dataset(tr_images, tr_oh_labels, batch_size=BATCH_SIZE, augmentor=None, shuffle=True,
                         pre_func=zero_one_scalar)
val_ds = Alphabet_Dataset(val_images, val_oh_labels, batch_size=BATCH_SIZE, augmentor=None, shuffle=False,
                          pre_func=zero_one_scalar)

model = create_alexnet(in_shape=(IMAGE_SIZE, IMAGE_SIZE, COLOR), n_classes=CLASSES,
                       kernel_regular=regularizers.l2(l2=REGULARIZER), optimizer=SGD(learning_rate=LEARNING_RATE))
...
test_ds = Alphabet_Dataset(test_images, test_oh_labels, batch_size=BATCH_SIZE, augmentor=None, shuffle=False,
